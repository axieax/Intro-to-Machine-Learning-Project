...
def train(model, lr, lamb, train_data, zero_train_data, valid_data, num_epoch, k):
    ...
    n_train_obs = torch.count_nonzero(train_data)
    ...
    plot.plot_3d(
        num_epoch, train_losses, k, "Training Loss", file_name="q3d-train-loss.png"
    )
    plot.plot_3d(
        num_epoch, valid_losses, k, "Validation Loss", file_name="q3d-valid-loss.png"
    )

    plot.plot_3d_avg_comparison(
        num_epoch, avg_train_losses, avg_valid_losses, k, file_name="q3d-avg-loss.png"
    )
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
