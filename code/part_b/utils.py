...
# Metadata Loading
def load_student_data(root_dir="/data"):
    ...
    with open(path, mode="r") as f:
        ...
        for line in f:
            ...
            if len(line) > 0:
                ...
                data = {
                    "gender": int(line[1]),
                    # "date_of_birth": date.fromisoformat(line[2].split(" ")[0])
                    # if line[2] != ""
                    # else np.nan,
                    "age": (REF_DATE - date.fromisoformat(line[2].split(" ")[0])).days
                    / 365
                    if line[2] != ""
                    else 0.0,
                    "premium": int(float(line[3])) + 1 if line[3] != "" else 0,
                }
                student_data_map[user_id] = data
